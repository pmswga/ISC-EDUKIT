...
ALTER TABLE `parent_child` ADD CONSTRAINT relations_pc FOREIGN KEY (id_type_relation) REFERENCES `relations` (id_relation) ON UPDATE CASCADE;
...
ALTER TABLE `student_test` ADD CONSTRAINT students_tests FOREIGN KEY (`id_student`) REFERENCES `students` (`id_student`) ON UPDATE CASCADE;

/* Связка таблицы "student_test" с таблицей "tests" */
ALTER TABLE `student_test` ADD CONSTRAINT stud_test FOREIGN KEY (`id_test`) REFERENCES `tests` (`id_test`) ON UPDATE CASCADE;
