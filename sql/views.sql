...
CREATE VIEW v_Tests (id_test, snp, email, test_name, subject_id, subject) as
  SELECT t.id_test, CONCAT(u.second_name, ' ', LEFT(u.first_name, 1), '. ', LEFT(u.patronymic, 1), '.'), u.email, t.caption, s.id_subject, s.description as subject
  FROM `tests` t
